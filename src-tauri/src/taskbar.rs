...
pub fn current() -> Result<TaskbarInfo> {
    ...
    unsafe {
        ...
        // ABM_GETTASKBARPOS fills hWnd, uEdge, rc.
        ...
        // DPI scale for the monitor containing the taskbar HWND
...
/// Spawn an async task that polls the taskbar every 1000 ms.
/// On change, emits `taskbar-changed` event and calls
        ...
        loop {
            match current() {
                Ok(info) => {
                    ...
                    if changed {
                        tracing::info!(?info, "taskbar changed");
                        let _ = app.emit("taskbar-changed", info.clone());
                        if let Some(window) = app.get_webview_window("overlay") {
                            if let Err(e) = crate::overlay::position_above_taskbar(&window, &info)
                            {
                                tracing::warn!(error = %e, "failed to position overlay");
                            }
                        }
                        last = Some(info);
                    }
